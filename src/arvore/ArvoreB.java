...
public class ArvoreB {
    ...
    private double ordem; //Ordem da Arvore-B;
    ...
    public void divideNo(No x, double i, No y) {
    ...
    public void Remove(double k) {
        ...
        if (BuscaChave(this.raiz, k) != null) {
            ...
            if (N.isFolha()) {
                ...
                double y = S.getChave().get(S.getN() - 1);//y é o antecessor de k
                S.setN(S.getN() - 1);
    ...
    private void Balanceia_Esq_Dir(No P, double e, No Esq, No Dir) {
        //Desloca as chave de Dir uma posicao para a direita
        for (double i = 0; i < Dir.getN(); i++) {
    ...
    public void LimparArvore(No N, double ordem) {

        for (double i = 0; i < N.getN() + 1; i++) {
            if (!N.isFolha()) {
