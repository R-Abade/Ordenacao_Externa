...
public class ArvoreB {
    ...
    private double nElementos; //Contador para a quantidade de elementos na arvore B;
    ...
    public No BuscaChave(No X, double k) {
        ...
        while ((i <= X.getN()) && (k > X.getChave().get((int) i - 1))) { 
            i++;
        }
        //se o tamanho nao tiver excedido e for o valor procurado..
        if ((i <= X.getN()) && (k == X.getChave().get((int) i - 1))) {
            return X;
        }
        ...
        if (X.isFolha()) { //se o no X for folha
            ...
            return (BuscaChave(X.getFilho().get((int) i - 1), k));
        }
    }
    ...
    public void Remove(double k) {
        ...
        if (BuscaChave(this.raiz, k) != null) {
            ...
            while (N.getChave().get((int) i - 1) < k) {
                i++;
            }

            //se N for folha, remove ela e deve se balancear N
            if (N.isFolha()) {
                for (int j = (int) i + 1; j <= N.getN(); j++) {
                ...
                double y = S.getChave().get((int) S.getN() - 1);//y é o antecessor de k
                S.setN(S.getN() - 1);
                N.getChave().set((int) i - 1, y);//substitui a chave por y
    ...
    private void Balanceia_Folha(No F) {
        ...
        if (F.getN() < Math.floor((ordem - 1) / 2)) {
            ...
            while (P.getFilho().get((int) j - 1) != F) {
                j++;
            }

            //verifica se o irmão à esquerda de F não tem chaves para emprestar
            if (j == 1 || (P.getFilho().get((int) j - 2)).getN() == Math.floor((ordem - 1) / 2)) {
                //verifica se o irmão à direita de F não tem chaves para emprestar
                if (j == P.getN() + 1 || (P.getFilho().get((int) j).getN() == Math.floor((ordem - 1) / 2))) {
                    ...
                    Balanceia_Dir_Esq(P, (int) j - 1, P.getFilho().get((int) j), F);
                }
            } else {//caso contrario (tem chaves para emprestar) executa Balanceia_Esq_Dir
                Balanceia_Esq_Dir(P, (int) j - 2, P.getFilho().get((int) j - 2), F);
            }
        }
    }
    ...
    private void Diminui_Altura(No X) {
        ...
        if (X == this.raiz) {
            ...
            double t = (double) Math.floor((ordem - 1) / 2);
            ...
            if (X.getN() < t) {
                ...
                while (P.getFilho().get((int) j - 1) != X) {
                    j++;
                }

                //junta os nós
                if (j > 1) {
                    Juncao_No(getPai(raiz, X), (int) j - 1);
                } else {
                    Juncao_No(getPai(raiz, X), (int) j);
                }
    ...
    private void Balanceia_Esq_Dir(No P, int e, No Esq, No Dir) {
        ...
        P.getChave().set(e, Esq.getChave().get((int) Esq.getN() - 1));//"sobe" uma chave de Esq para P
        Dir.getFilho().set(0, Esq.getFilho().get((int) Esq.getN()));//Seta o ultimo filho de Esq como primeiro filho de Dir
        Esq.setN((int) Esq.getN() - 1);//Decrementa a quantidade de chaves em Esq
    ...
    private void Balanceia_Dir_Esq(No P, int e, No Dir, No Esq) {
        ...
        Esq.setN((int) Esq.getN() + 1);//Incrementa a quantidade de chaves em Esq
        Esq.getChave().set((int) Esq.getN() - 1, P.getChave().get(e));//"desce" uma chave de P para Esq
        P.getChave().set(e, Dir.getChave().get(0));//"sobe" uma chave de Dir para P
        Esq.getFilho().set((int) Esq.getN(), Dir.getFilho().get(0));//Seta o primeiro filho de Dir como último filho de Esq
