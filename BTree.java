...
public class BTree {
        ...
        public void add(int key, Object object) {
                ...
                if (!update(mRootNode, key, object)) {
                        if (rootNode.mNumKeys == (2 * T - 1)) {
                                ...
                                splitChildNode(newRootNode, 0, rootNode); // Split rootNode and move its median (middle)
                                                                          // key up into newRootNode.
                                insertIntoNonFullNode(newRootNode, key, object); // Insert the key into the B-Tree with
                                                                                 // root newRootNode.
                        } else {
                                insertIntoNonFullNode(rootNode, key, object); // Insert the key into the B-Tree with
                                                                              // root rootNode.
                        }
                }
        }
        ...
        public void delete(Node node, int key) {
                if (node.mIsLeafNode) { // 1. If the key is in node and node is a leaf node, then delete the key from
                        ...
                        int i;
                        if ((i = node.binarySearch(key)) != -1) { // 2. If node is an internal node and it contains the
                                                                  // key... (key is i-th key of node if node contains
                                                                  // key)
                                Node leftChildNode = node.mChildNodes[i];
                                Node rightChildNode = node.mChildNodes[i + 1];
                                if (leftChildNode.mNumKeys >= T) { // 2a. If the predecessor child node has at least T
                                                                   // keys...
                                        Node predecessorNode = leftChildNode;
                                        Node erasureNode = predecessorNode; // Make sure not to delete a key from a node
                                                                            // with only T - 1 elements.
                                        while (!predecessorNode.mIsLeafNode) { // Therefore only descend to the previous
                                                                               // node (erasureNode) of the predecessor
                                ...
                                } else if (rightChildNode.mNumKeys >= T) { // 2b. If the successor child node has at
                                                                           // least T keys...
                                        Node successorNode = rightChildNode;
                                        Node erasureNode = successorNode; // Make sure not to delete a key from a node
                                                                          // with only T - 1 elements.
                                        while (!successorNode.mIsLeafNode) { // Therefore only descend to the previous
                                                                             // node (erasureNode) of the predecessor
                                                                             // node and delete the key using 3.
                                        ...
                                        moveKey(node, i, RIGHT_CHILD_NODE, leftChildNode, medianKeyIndex); // Delete i's
                                                                                                           // right
                                                                                                           // child
                                                                                                           // pointer
                                                                                                           // from node.
                                ...
                                if (childNode.mNumKeys == T - 1) {
                                        ...
                                        Node rightChildSibling = (i + 1 <= node.mNumKeys) ? node.mChildNodes[i + 1]
                                                        : null;
                                        if (leftChildSibling != null && leftChildSibling.mNumKeys >= T) { // 3a. The
                                                                                                          // left
                                                                                                          // sibling has
                                                                                                          // >= T
                                                                                                          // keys...
                                                // Move a key from the subtree's root node down into childNode along
                                                // with the appropriate child pointer.
                                                // Therefore, first shift all elements and children of childNode right
                                                // by 1.
                                                childNode.shiftRightByOne();
                                                childNode.mKeys[0] = node.mKeys[i - 1]; // i - 1 is the key index in
                                                                                        // node that is smaller than
                                                ...
                                                node.mKeys[i - 1] = leftChildSibling.mKeys[leftChildSibling.mNumKeys
                                                                - 1];
                                                node.mObjects[i - 1] = leftChildSibling.mObjects[leftChildSibling.mNumKeys
                                                                - 1];

                                                // Remove the key from the left sibling along with its right child node.
                                                leftChildSibling.remove(leftChildSibling.mNumKeys - 1,
                                                                RIGHT_CHILD_NODE);
                                        } else if (rightChildSibling != null && rightChildSibling.mNumKeys >= T) { // 3a.
                                                                                                                   // The
                                                                                                                   // right
                                                                                                                   // sibling
                                                                                                                   // has
                                                ...
                                                // Move a key from the subtree's root node down into childNode along
                                                // with the appropriate child pointer.
                                                childNode.mKeys[childNode.mNumKeys] = node.mKeys[i]; // i is the key
                                                                                                     // index in node
                                                                                                     // that is bigger
                                                                                                     // than childNode's
                                                                                                     // biggest key.
                                                ...
                                                if (!childNode.mIsLeafNode) {
                                                        childNode.mChildNodes[childNode.mNumKeys
                                                                        + 1] = rightChildSibling.mChildNodes[0];
                                                }
                                                childNode.mNumKeys++;
                                                ...
                                                if (leftChildSibling != null) {
                                                        ...
                                                        moveKey(node, i - 1, LEFT_CHILD_NODE, childNode,
                                                                        medianKeyIndex); // i - 1 is the median key
                                                                                         // index in node when merging
                                                                                         // with the left sibling.
                                                } else if (rightChildSibling != null) {
                                                        int medianKeyIndex = mergeNodes(childNode, rightChildSibling);
                                                        moveKey(node, i, RIGHT_CHILD_NODE, childNode, medianKeyIndex); // i
                                                                                                                       // is
                                                                                                                       // the
                                                                                                                       // median
                                                                                                                       // key
        ...
        int mergeNodes(Node dstNode, Node srcNode) {
                ...
                if (srcNode.mKeys[0] < dstNode.mKeys[dstNode.mNumKeys - 1]) {
                        ...
                        if (!dstNode.mIsLeafNode) {
                                dstNode.mChildNodes[srcNode.mNumKeys + dstNode.mNumKeys
                                                + 1] = dstNode.mChildNodes[dstNode.mNumKeys];
                        }
        ...
        void validate() throws Exception {
                ...
                for (int i = 0; i < array.size() - 1; i++) {
                        if (array.get(i) >= array.get(i + 1)) {
                                throw new Exception("B-Tree invalid: " + array.get(i) + " greater than "
                                                + array.get(i + 1));
                        }
                }
        }
